...
class MockWarble : public WarbleServiceAbstraction {
 public:
  ...
  MOCK_METHOD3(WarbleText, std::string(const std::string &user_name,
                                       const std::string &text,
                                       const StringOptional &reply_to));
  MOCK_METHOD2(Follow, void(const std::string &user_name,
...
//           Event function will return a payload with an empty
//           RegisteruserReply
TEST_F(FuncPlatformTest,
       shouldReturnPayloadWithRegisteruserReplyWhenExectueEventType1) {
  int event_type = 1;
...
//           Event function will return a payload with an WarbleReply with the
//           warble that has expected fields value.
TEST_F(
    FuncPlatformTest,
    shouldReturnPayloadWithWarbleReplyWhenExectueEventType2AndWarbleRequestWithoutReplyTo) {
  int event_type = 2;
  ...
  EXPECT_CALL(*mock_warble_, WarbleText("Harry Potter", "It's my first warble.",
                                        StringOptional("")))
      .Times(1)
      .WillOnce(Return("1"));
  Payload reply_payload = service_->Execute(event_type, payload);
...
TEST_F(
    FuncPlatformTest,
    shouldReturnPayloadWithWarbleReplyWhenExectueEventType2AndWarbleRequestWithReplyTo) {
  int event_type = 2;
  WarbleRequest request;
  WarbleReply reply;
  request.set_username("Harry Potter");
...
TEST_F(FuncPlatformTest,
       shouldReturnPayloadWithFollowReplyWhenExectueEventType3) {
  int event_type = 3;
...
//           Event function will return a payload with an ProfileReply that has
//           expected followings and followers.
TEST_F(FuncPlatformTest,
       shouldReturnPayloadWithProfileReplyWhenExectueEventType5) {
  int event_type = 5;
...
TEST_F(
    FuncPlatformTest,
    shouldReturnEmptyVectorOfWarbleWhenCallExecuteWithEvetType4AndWarbleIdWithoutAnyReplies) {
  int event_type = 4;
  ReadRequest request;
  ReadReply reply;
...
TEST_F(
    FuncPlatformTest,
    shouldReturnVectorOfWarbleWhenCallExecuteWithEvetType4AndWarbleIdWithReplies) {
  int event_type = 4;
  ReadRequest request;
  ReadReply reply;
